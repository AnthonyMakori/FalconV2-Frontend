  ...
  useEffect(() => {
    const fetchTrendingMovie = async () => {
      try {
        ...
        if (list.length > 0) {
          // Random movie from top 5
          const randomIndex = Math.floor(Math.random() * Math.min(5, list.length))
  ...
  return (
    <div className="relative w-full h-[60vh] overflow-hidden rounded-lg">
      ...
      <div className="relative h-full flex flex-col justify-center p-6 md:p-12 max-w-3xl">
        ...
        <p className={cn("text-base text-muted-foreground mb-6", "line-clamp-3 md:line-clamp-4")}>
